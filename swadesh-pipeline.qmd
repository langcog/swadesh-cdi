...
cv_res <- lapply(train_langs, \(lang) {
  ...
  bind_rows(prod_cors, cat_cors, rand_cors)
}) |> bind_rows()
...
prod_sum <- prod_pars |> 
  filter(language %in% train_langs) |> 
  group_by(uni_lemma) |> # now 2735, was 2007 with only uni_lemma
  summarise(num_langs = n(),
            mean_d = mean(d, na.rm = TRUE),
            sd_d = sd(d, na.rm = TRUE))
...
# TODO: Alvin, is also grouping by category alright? maybe we just take modal uni category?
prod_sum <- prod_pars |> 
  filter(language %in% train_langs) |> 
  group_by(uni_lemma, category) |> # now 2735, was 2007 with only uni_lemma
  summarise(num_langs = n(),
            mean_d = mean(d, na.rm = TRUE),
            sd_d = sd(d, na.rm = TRUE))
...
```{r, fig.width=6, fig.height=4}
# TODO: proportional clearly has too many items..
ggplot(gen_res_sum,
...
```{r}
swadesh_sublist <- swadesh_sublist %>% left_join(xldf_clean %>% filter(language=="English (American)") %>% select(uni_lemma, category))
swad_cats <- swadesh_sublist %>% group_by(category) %>% summarise(n = n()) %>% arrange(desc(n)) %>%
  mutate(freq = n / sum(n))
swad_cats
```
