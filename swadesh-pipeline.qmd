...
CV results show that $k=25$ is the optimal value for both the unstratified and 2 difficulty strata methods, so we use this to construct the final list for generalization testing.
For the category proportional method, k=26 (which is quite similar).
...
category_sublist26 <- make_proportional_sublist(prod_sum, S_LEN, 26) # best k for category
...
intersect(swadesh_sublist$uni_lemma, strat2_sublist$uni_lemma) # 96 - !
intersect(swadesh_sublist$uni_lemma, category_sublist$uni_lemma) # 64
intersect(swadesh_sublist$uni_lemma, category_sublist26$uni_lemma) # 62

setdiff(swadesh_sublist$uni_lemma, category_sublist$uni_lemma) # a lot of extra sounds and animals
