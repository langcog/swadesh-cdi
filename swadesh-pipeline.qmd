...
prod_pars <- xldf_clean |> 
  ...
  select(uni_lemma, language, uid, category, language, d, a1) |>
...
create_cors <- function(full_model, full_fscores, lang, prod_sum, prod_test, 
  ...
  compute_metrics <- function(k, sublist) {
    fscore_cor <- if (is.element(k, BEST_Ks)) {
      tryCatch({
  ...
  create_sublist <- function(k) {
    if (sublist_name == "syntactic") {
      sublist_func(prod_sum, S_LEN, k, syntactic_props)
    } else if (sublist_name == "category") {
      sublist_func(prod_sum, S_LEN, k, category_props)
    } else if (is.null(bins)) {
      sublist_func(prod_sum, S_LEN, k)
    } else {
      sublist_func(prod_sum, S_LEN, k, n_bins = bins)
    }
  }
  
  k_values <- 2:(length(train_langs) - 1)
  
  if (sublist_name == "Random") {
    result <- lapply(k_values, \(k) {
      sapply(1:N_RAND, \(comp) {
        sublist <- create_sublist(k)
        compute_metrics(k, sublist)
      }) |> 
        t() |> 
    ...
    result <- sapply(k_values, \(k) {
      ...
      `colnames<-`(c("num_overlap", "difficulty_cor", "rmse", "fscore_cor")) |> 
      as_tibble() |> 
      mutate(run = NA, k = k_values, language = lang, sublist = sublist_name)
  }
...
cv_res <- lapply(train_langs, \(lang) {
  ...
  create_category_summary <- function(data, category_column) {
    cats <- data |> 
      filter(language %in% train_langs, language != lang) |> 
      group_by(uni_lemma) |> 
      count(.data[[category_column]]) |> 
      arrange(desc(.data[[category_column]])) |> 
      slice(1)
    ...
    data |> 
      ...
      summarise(num_langs = n(),
                ...
                .groups = "drop") |> 
      left_join(cats |> select(-n), by = join_by(uni_lemma)) |>
      rename(category = all_of(category_column))
  }
  
  prod_semantic_sum <- create_category_summary(prod_pars, "semantic_category")
  prod_syntactic_sum <- create_category_summary(prod_pars, "syntactic_category")
...
10/7/24: We prefer difficulty correlation over RMSE since the samples (and thus language parameters) are not directly comparable (but should be correlated)
...
rand_cors <- lapply(gen_langs, \(lang) {
  rand_cors <- sapply(1:N_RAND, \(comp) {
    ...
    get_sumscore_cor(sublist, xldf, all_prod, lang) # ...or xldf_clean ?
  }) |> t() |> 
    `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
    as_tibble() |> 
    mutate(run = 1:N_RAND,
...
# Final list: syntactic-stratified + 10-item extension
...
swad_extension_unis <- c("try", "under", "in", "out", 
                         ...
                         "all", "many", "not", "a lot")
swad_extension <- prod_syntactic_sum |> filter(is.element(uni_lemma, swad_extension_unis))
swad_extended <- syntactic_sublist |> bind_rows(swad_extension)

swad_ext_cors <- sapply(gen_langs, \(lang) {
    get_sumscore_cor(swad_extended, xldf_clean, all_prod, lang)
  }) |> t() |>
    `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
    as_tibble(rownames = "language") |> 
    mutate(sublist = "S-CDI110")

# Combine all results
gen_res <- bind_rows(unstrat_cors, syntactic_cors, category_cors, 
                     rand_cors, strat2_cors, swad_ext_cors)
...
gen_res_sum |> group_by(sublist) |>
  summarise(num_overlap = mean(num_overlap),
            sumscore_cor = mean(sumscore_cor), 
            sumscore_cor_t = mean(sumscore_cor_t)) |>
  arrange(desc(sumscore_cor))
```

```{r, generate-candidate-swadesh-lists, echo=F}
...
```{r}
aov(sumscore_cor ~ sublist, data=gen_res_sum)
aov(num_overlap ~ sublist, data=gen_res_sum)
...
From [Bella, Batsuren, and Giunchiglia (2021)](http://ukc.disi.unitn.it/index.php/lexsim/) [@bella2021]
...
Missing languages: ASL, BSL, Estonian, Latvian, Kiswahili (or Swahili), or Kigiriama
...
matches = intersect(unique(lang_sims$LangName_1), unique(xldf_clean$lang_mapped)) # 26
langs_to_remove <- setdiff(unique(xldf_clean$lang_mapped), unique(lang_sims$LangName_1))

# Replace NA values if the language doesn't have a mapping
xldf_clean$lang_mapped[is.na(xldf_clean$lang_mapped)] <- xldf$language[is.na(xldf_clean$lang_mapped)]
...
# Compare dendrogams of historic language relations and CDI word difficulty
...
lang_sim_d <- dist(scale(lang_sims_mat))
lang_sim_cl <- hclust(lang_sim_d, method = "ward.D2") # method = "average"
lang_sim_dend <- as.dendrogram(lang_sim_cl)

pdf("lang_sim_dendrogram.pdf", height=8, width=12)
dendextend::plot_horiz.dendrogram(lang_sim_dend)
dev.off()

swad_lang_sims_long <- xldf_clean %>% 
...
swad_dif_mat <- get_xling_difficulty_similarity(swad_lang_sims_long) # for Swadesh list items
#dif_cors # for all items

swad_d <- dist(scale(swad_dif_mat))
swad_cl <- hclust(swad_d, method = "ward.D2") # method = "average"
swad_dend <- as.dendrogram(swad_cl)

pdf("swad_sim_dendrogram.pdf", height=8, width=12)
...
```{r}
pdf("swad_dif_vs_lang_sim_tanglegram.pdf", height=8, width=14)
dends |> untangle(method = "step1side") |> 
  tanglegram(
    lab.cex = 1, margin_inner = 8,
    highlight_distinct_edges = FALSE, # Turn-off dashed lines
    common_subtrees_color_lines = FALSE, # Turn-off line colors
    common_subtrees_color_branches = TRUE) |> # Color common branches 
  entanglement() # alignment quality (lower is better)
dev.off()
# 0.26
...
cor_cophenetic(dends) # .18

# Baker correlation coefficient
cor_bakers_gamma(dends) # .20
```


```{r}
bakers_gamma_perm_test <- function(dend1, dend2, R=200) {
  the_cor <- cor_bakers_gamma(dend1, dend2)
  the_cor2 <- cor_bakers_gamma(dend2, dend1)
  cor_bakers_gamma_results <- numeric(R)
  dend_mixed <- dend1
  for(i in 1:R) {
     dend_mixed <- sample.dendrogram(dend_mixed, replace = FALSE)
     cor_bakers_gamma_results[i] <- cor_bakers_gamma(dend2, dend_mixed)
  }
...
bakers_gamma_perm_test(swad_dend, lang_sim_dend)
```
