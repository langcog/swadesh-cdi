...
# Swadesh 
unstrat_sublist <- make_swadesh_sublist(prod_sum, S_LEN, BEST_K)
unstrat_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(unstrat_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "unstratified")

strat2_sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, BEST_K, n_bins=2)
strat2_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(strat2_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "2 strata")

strat3_sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, BEST_K, n_bins=3)
strat3_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(strat3_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "3 strata")

strat4_sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, BEST_K, n_bins=4)
strat4_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(strat4_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "4 strata")

strat5_sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, BEST_K, n_bins=5)
strat5_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(strat5_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "5 strata")
...
cat_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(category_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "category")


gen_res <- bind_rows(unstrat_cors, cat_cors, rand_cors, 
                     strat2_cors, strat3_cors, strat4_cors, strat5_cors)
