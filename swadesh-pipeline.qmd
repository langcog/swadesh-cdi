...
create_cors <- function(full_model, full_fscores, lang, prod_sum, prod_test, 
  ...
  if (sublist_name=="Random") {
    ...
    sapply(2:(length(train_langs)-1), \(k) {
      sublist <- sublist_func(prod_sum, S_LEN, k, cat_props) # PROBLEM:
      # Slovak is missing 4 categories of words
      # setdiff(cat_props$category, unique(slo$category))
      # "games_routines" "helping_verbs"  "places"         "time_words" 
      # setdiff(unique(slo$category), cat_props$category) # extra Slovak categories: "descriptive_words (adverbs)" "other" 
      # Note: "today" is in "descriptive_words (adverbs)" ...
      # problem: this is empty:
      # make_proportional_sublist(prod_sum, 100, 23, cat_props)
      if (is.element(k, BEST_Ks)) { 
        fscore_cor <- tryCatch({
          get_fscore_cor(sublist, xldf_clean, lang, full_fscores)
        }, error = function(e) {
      ...
      c(get_difficulty_metrics(sublist, prod_test), fscore_cor) # <- error is here: 'uni_lemma' column not in sublist
    }) |> t() |> 
      `colnames<-`(c("num_overlap", "difficulty_cor", "rmse", "fscore_cor")) |> 
      as_tibble() |> 
...
cv_res <- lapply(train_langs, \(lang) {
  ...
  prod_cats <- prod_pars |> 
    ...
    group_by(uni_lemma) |> 
    count(category) |> # ToDo: is this category OK for syntactic??
  ...
  syn_cors <- create_cors(full_model, full_fscores, lang, prod_sum, prod_test, 
...
t.test(subset(cv_res_sum, sublist=="category" & k==25)$fscore_cor - 
       subset(cv_res_sum, sublist=="Random" & k==23)$fscore_cor) # n.s.
t.test(subset(cv_res_sum, sublist=="category" & k==25)$fscore_cor - 
       subset(cv_res_sum, sublist=="unstratified" & k==23)$fscore_cor) # n.s.
t.test(subset(cv_res_sum, sublist=="category" & k==25)$fscore_cor - 
       subset(cv_res_sum, sublist=="2 strata" & k==26)$fscore_cor) # n.s.
t.test(subset(cv_res_sum, sublist=="Random" & k==23)$fscore_cor - 
       subset(cv_res_sum, sublist=="2 strata" & k==26)$fscore_cor) # n.s.

t.test(subset(cv_res_sum, sublist=="unstratified" & k==23)$fscore_cor - 
       subset(cv_res_sum, sublist=="2 strata" & k==26)$fscore_cor) # p<.001 (2 strata .002 better)

t.test(subset(cv_res_sum, sublist=="unstratified" & k==23)$fscore_cor - 
       subset(cv_res_sum, sublist=="2 strata" & k==26)$fscore_cor)
...
CV results show that $k=26$ is the optimal value for the 2 difficulty strata method, $k=25$ is optimal for the category-stratified method, and $k=23$ is optimal for the unstratified method.
...
# ToDo: update this to accept semantic or syntactic category prop
generate_sublist_cors <- function(sublist_function, bins = NULL, sublist_name, this_k) {
  sublist <- if (is.null(bins)) {
    sublist_function(prod_sum, S_LEN, this_k)
  } else {
    sublist_function(prod_sum, S_LEN, this_k, bins)
  }
...
strat2_cors <- generate_sublist_cors(make_binned_swadesh_sublist, 2, sublist_name = "2 strata", this_k = 26)

category_cors <- generate_sublist_cors(make_proportional_sublist, sublist_name = "category", this_k = 25)

syntactic_cors <- generate_sublist_cors(make_proportional_sublist, sublist_name = "syntactic", this_k = 25) # ToDo: which k?
...
# Combine all results
gen_res <- bind_rows(unstrat_cors, category_cors, rand_cors, strat2_cors)
...
unstrat_sublist <- make_swadesh_sublist(prod_sum, S_LEN, 23)
category_sublist <- make_proportional_sublist(prod_sum, S_LEN, 25, category_props) 
syntactic_sublist <- make_proportional_sublist(prod_sum, S_LEN, 25, syntactic_props) # ToDo: update with cross-validated k
strat2_sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, 26, n_bins=2)

table(unstrat_sublist$category) # 18 categories - few action words, more animals
table(category_sublist$category) # 22 cats - more actions, 
table(strat2_sublist$category) # 17 cats
...
setdiff(unique(category_sublist$category), unique(strat2_sublist$category))
