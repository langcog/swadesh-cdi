...
cv_res <- lapply(train_langs, \(lang) {
  ...
  # Swadesh - best words in 3 difficulty strata
  bin3_cors <- sapply(2:(length(train_langs)-1), \(k) {
    sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, k, n_bins = 3)
    if (k == BEST_K) {
      fscore_cor <- get_fscore_cor(sublist, xldf_clean, lang, full_fscores)
    } else {
      fscore_cor <- NA
  ...
  # Swadesh - category proportion match
  cat_cors <- sapply(2:(length(train_langs)-1), \(k) {
    sublist <- make_proportional_sublist(prod_sum, S_LEN, k)
    if (k == BEST_K) {
      fscore_cor <- get_fscore_cor(sublist, xldf_clean, lang, full_fscores)
    } else {
      fscore_cor <- NA
  ...
  bind_rows(prod_cors, bin3_cors, cat_cors, rand_cors)
}) |> bind_rows()
...
cv_output[which(cv_output$difficulty_cor == max(cv_output$difficulty_cor)),] 
# k=23, Swadesh sublist, num_overlap=90.5, diff_cor=.841
```
...
t.test(subset(cv_res_sum, sublist=="Swadesh" & k==BEST_K)$fscore_cor - 
       subset(cv_res_sum, sublist=="Random" & k==BEST_K)$fscore_cor) # n.s.
t.test(subset(cv_res_sum, sublist=="Proportional" & k==BEST_K)$fscore_cor - 
       subset(cv_res_sum, sublist=="Random" & k==BEST_K)$fscore_cor) # n.s.
t.test(subset(cv_res_sum, sublist=="Proportional" & k==BEST_K)$fscore_cor - 
       subset(cv_res_sum, sublist=="Swadesh" & k==BEST_K)$fscore_cor) # n.s.
...
# Proportional
proportional_sublist <- make_proportional_sublist(prod_sum, S_LEN, BEST_K)
prop_cors <- sapply(gen_langs, \(lang) {
  get_sumscore_cor(proportional_sublist, xldf, all_prod, lang)
}) |> t() |>
  `colnames<-`(c("num_overlap", "sumscore_cor")) |> 
  as_tibble(rownames = "language") |> 
  mutate(sublist = "Proportional")


gen_res <- bind_rows(prod_cors, prop_cors, rand_cors)
...
Proportional is also no different than random in terms of sumscore correlation:
```{r}
t.test(gen_res_sum |> filter(sublist == "Proportional") |> pull(sumscore_cor),
       gen_res_sum |> filter(sublist == "Random") |> pull(sumscore_cor),
       paired = TRUE)
```
...
```{r}
t.test(gen_res_sum |> filter(sublist == "Proportional") |> pull(num_overlap),
       gen_res_sum |> filter(sublist == "Random") |> pull(num_overlap),
       paired = TRUE)
```
...
```{r}
swad_cats <- swadesh_sublist |> mutate(sublist = "Swadesh") |>
  bind_rows(proportional_sublist |> mutate(sublist = "Proportional")) |>
...
```{r}
swadesh_sublist |> mutate(sublist = "Swadesh") |>
  bind_rows(proportional_sublist |> mutate(sublist = "Proportional")) |>
...
```{r}
cv_res_prop <- lapply(train_langs, \(lang) {
  message(glue("Calculating for {lang}..."))
  ...
  # bind_rows(cat_cors, rand_cors)
}) |> bind_rows()

cv_res_sum_prop <- cv_res_prop |> 
  group_by(k, language, sublist) |> 
  summarise(num_overlap = mean(num_overlap),
            difficulty_cor = mean(difficulty_cor),
            fscore_cor = mean(fscore_cor)) |> 
...
cv_output_prop <- cv_res_sum_prop |> 
  group_by(k, sublist) |> 
  summarise(num_overlap_mean = mean(num_overlap, na.rm = TRUE),
            num_overlap_sd = sd(num_overlap, na.rm = TRUE),
            difficulty_cor_mean = mean(difficulty_cor, na.rm = TRUE),
            difficulty_cor_sd = sd(difficulty_cor, na.rm = TRUE))
```
