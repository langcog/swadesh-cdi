...
```{r}
# FixMe: prod_pars now has both semantic_category and syntactic_category - we need each for the two methods
prod_cats <- prod_pars |> 
  filter(language %in% train_langs) |> 
  group_by(uni_lemma) |> 
  count(semantic_category) |> 
...
strat2_cors <- generate_sublist_cors(make_binned_swadesh_sublist, 2, sublist_name = "2 strata", this_k = 27)
