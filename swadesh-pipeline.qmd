...
cv_res <- lapply(train_langs, \(lang) {
  ...
  prod_cors <- sapply(2:(length(train_langs)-1), \(k) {
    ...
    mutate(run = NA,
           ...
           language = lang,
           sublist = "unstratified")
  
  # Swadesh - difficulty stratified (2 bins)
  bin2_cors <- sapply(2:(length(train_langs)-1), \(k) {
    sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, k, n_bins = 2)
    if (k == BEST_K) {
      fscore_cor <- get_fscore_cor(sublist, xldf_clean, lang, full_fscores)
    } else {
      fscore_cor <- NA
  ...
  # Swadesh - difficulty stratified (3 bins)
  ...
  # Swadesh - difficulty stratified (4 bins)
  bin4_cors <- sapply(2:(length(train_langs)-1), \(k) {
    sublist <- make_binned_swadesh_sublist(prod_sum, S_LEN, k, n_bins = 4)
    if (k == BEST_K) {
      fscore_cor <- get_fscore_cor(sublist, xldf_clean, lang, full_fscores)
    } else {
      fscore_cor <- NA
  ...
  bind_rows(prod_cors, bin2_cors, bin3_cors, bin4_cors, cat_cors, rand_cors)
}) |> bind_rows()
