...
make_proportional_sublist <- function(prod_sum, list_size, k, cat_props) {
  cat_props_rounded <- cat_props |>
    mutate(
      prop = mean_prop * list_size,
      count = floor(prop),
      remainder = prop - count
    ) |>
    arrange(desc(remainder)) |>
    mutate(
      add = c(rep(1, list_size - sum(count)), rep(0, n() - list_size + sum(count))),
      count = count + add
    ) |>
  ...
  sublist <- prod_sum |>
    ...
    filter(!is.na(count)) |>
    mutate(data = map2(data, count, \(d, c) {
      slice_head(d, n = c)
    })) |>
    unnest(data)
  ...
  if (nrow(sublist) < list_size) {
    ...
    additional_items <- prod_sum |>
      ...
      filter(!is.na(wanted)) |>
      mutate(data = map2(data, wanted, \(d, c) {
        slice_head(d, n = c)
      })) |>
      unnest(data)
    # failure mode?
