...
get_cross_ling_difficulty_cors <- function(xldf) {
  ...
  for (l1 in languages) {
    for (l2 in languages) {
      tmp <- xldf %>%
        filter(language == l1 | language == l2, !is.na(d)) %>%
      ...
      prod_sims <- bind_rows(prod_sims, tibble(
        "Lang1" = l1, "Lang2" = l2,
...
run_swadesh_comparisons <- function(xldf, languages, swad_list, form = "WS", rand_comparisons = 100) {
  ...
  for (lang in languages) {
    ...
    xx <- xx %>% bind_rows(tibble(
      language = lang, sublist = "Swadesh",
...
run_comparisons <- function(xldf, languages, swad_list,
  ...
  for (lang in languages) {
    ...
    if ("sumscore_cor" %in% metrics) {
      ...
      sumscore_cor <- tibble(
        sublist = "Swadesh", run = NA,
        value = swad_sscor, N = length(swad_idx),
        mean_d = swad_d
      ) |>
        bind_rows(tibble(
          sublist = "random", run = 1:rand_comparisons,
