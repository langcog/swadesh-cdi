...
Short versions of the CDI:WS forms are also available [e.g., @marchman2023], each consisting of a set of around 100 items, often selected to generate scores that strongly correlate with scores on the full forms, while retaining representation across a broad set of semantic categories.
...
For example, the CDI has been adapted to only very few languages in India and Africa, despite the large number of speakers of some of these overlooked languages (e.g., Punjabi, Marathi, Telugu, Hausa, and Amharic, all of which have over 50 million speakers, see @eberhard2025). 
Moreover, the CDI has not been adapted to any of the thousands of languages spoken by indigenous peoples around the world.
...
More broadly, many of these languages are under-resourced, and lack a language resource ecosystem---not only vocabulary assessment instruments, but also corpora, acquisition descriptions, computational tools, and other resources supporting language development research [@hellwig2021].
The absence of these resources also makes CDI development more difficult---for example, examining corpora of child and child-directed language is a common method for selecting items for a new CDI [@jaruskova2023]; this approach would not be possible if such corpora do not exist, or are not appropriately pre-processed and made accessible.
...
Extending this work to the development of a universal CDI, or “Swadesh CDI,” would include many of the concepts that researchers have chosen to include on several CDI adaptations, and which have relatively similar difficulty across many languages. 
...
When a CDI form was administered, caregivers were asked to indicate for each vocabulary item on the instrument whether or not their child can recognizably produce (say) the given word in an appropriate context.
...
Moreover, there was a weak but significant relation between the number of forms a unilemma appears on and its cross-linguistic variability (`r report_cor(cor.test(uni_ag$d_sd, uni_ag$n))`, for unilemmas appearing more than once).
...
To satisfy the generalizability criterion, we aimed to choose unilemmas with low variability in their cross-linguistic difficulty---that is, unilemmas that are similarly difficult to learn across languages, operationalized as having the lowest standard deviation across languages in item difficulty.
...
Comparing the IRT parameters of the Swadesh-CDI unilemmas to the rest of the items (across all CDI forms) showed that the discrimination parameter (i.e., slope) of the S-CDI items did not significantly differ from the others, suggesting that the Swadesh items could measure ability as well as non-Swadesh unilemmas.
However, S-CDI items were significantly easier than other unilemmas (mean S-CDI $d=$ `r round(tmp[2,"d"], 2)`, others' mean $d=$ `r round(tmp[1,"d"], 2)`, `r report_ttest(swad_diff_ttest)`), which could result in older/more-skilled children being at ceiling on the test.
...
Recognizing the importance of assessing early language development, and the difficulty in adapting existing measures such as the CDI to understudied languages -- especially with the goal of enabling cross-linguistic comparisons of early learning -- this study leveraged psychometric models fitted to `r length(train_langs)` CDI datasets in order to find concepts that had low variability in their cross-linguistic difficulty, and that were frequently included on CDI forms. 
...
We strongly recommend that the Swadesh CDI list serve only as a starting point for developing a new CDI, and that researchers use an iterative process that takes into account local expert knowledge of the target language, as well as the potential multilingual learning context to select appropriate items.
