...
For example, the CDI has been adapted to only very few languages in India and Africa, despite the large number of speakers of some of these overlooked languages (e.g., Punjabi, Marathi, Telugu, Hausa, and Amharic, all of which have over 50 million speakers [@eberhard2025]). 
...
Moreover, there was a weak but significant relation between the number of forms a unilemma appears on and its cross-linguistic variability (`r report_cor(cor.test(uni_ag$d_sd, uni_ag$n))`, for uni-lemmas appearing more than once).
...
The fact that these various methods of selecting a Swadesh list tend to select many of the same unilemmas suggests that our choice may matter little.

## Characterizing the Swadesh-CDI 

```{r, fig.width=5.5, fig.cap=c("The 100-item Swadesh-CDI list by semantic category, along with a few recommended items from the unrepresented semantic categories (in gray)."), echo=FALSE}
...
swad_d_m %>% mutate(Swadesh = ifelse(SwadeshCDI, "Swadesh", "non-Swadesh")) %>%
  ...
  xlab("Language") + ylab("Mean item easiness")
```
...
ToDo REVIEW THIS RECOMMENDATION:
To mitigate the likelihood of a ceiling effect, we propose extending the S-CDI with 10 additional unilemmas chosen from the original Swadesh (1971) list, which were selected to fill gaps in the S-CDI, including pronouns (_I, you, we, this, that_), quantifiers (_all, many, not_), and question words (_who, what_).
