...
```{r, show-data, echo=F, results="asis"}
...
# }
# save(demo, file=here("data/all_demographic_data.Rdata"))

demo <- tibble()
WG_files <- list.files(path="../data/WG", pattern = "*.Rdata", full.names = T)
for(file in WG_files) {
  load(file)
  demo <- bind_rows(demo, d_demo %>% 
                      select(data_id, age, production, language))
}
save(demo, file=here("data/WG_combined_demo.Rdata"))
```

```{r combined-demo-data, echo=F}
# where was this made?
load(here("data/WS_combined_demo.Rdata"))
demo_ws <- demo %>% mutate(form = "WS")

load(here("data/WG_combined_demo.Rdata"))
demo <- demo %>% mutate(form = "WG") %>%
  bind_rows(demo_ws)
...
The Wordbank[^2] datasets consisted of CDI:WS production data for `r nrow(demo %>% filter(form=="WS"))` children aged 16--30 months and CDI_WG production data from `r nrow(demo %>% filter(form=="WG"))` children aged 8--18 months
...
lang_d_m <- xldf_clean %>% filter(!is.na(d)) %>% # one Slovak word has NA parms..
  group_by(language) %>% 
...
With these candidate lists, we compared the item difficulties in the held-out language with the mean item difficulties in the remaining training languages, and selected the value of k for which the correlation was the highest, $k = 23$.

Finally, we re-ran the selection procedure using $k = 23$ across all training languages (without holding out any language) to arrive at the final Swadesh CDI list, shown in Table 2.
...
# ToDo: remove entirely -- but load relevant dataframes from swadesh-pipeline.qmd
...
for(k in 2:31) {
  ...
  xx_g <- run_comparisons(xldf, train_langs |> sort(), swad_lists[[k]]$uni_lemma, 
...
save(swad_agg, swad_lists, swad_freq, file=here("data/intermediates/Swadesh_grid_search_final.Rdata"))
...
```{r plot-crossval-fscores, fig.width=4, fig.height=8}
